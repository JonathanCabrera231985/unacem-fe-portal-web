...
namespace Control
{
    public class Recepcion
    {
        ...
        XmlElement facturaXML, facturaXML_recibida;
        string doc_version = "";
        ...
        private int puerto;
        private bool ssl;
        private string compania = "IPAC S.A.";
            ...
            try
            {
                while (i_contador < intentos_autorizacion && b_respuesta == false && b_no_autorizado == false)
                {
                    ...
                    using (var autorizacionTrace = autorizacion)
                    {
                        ...
                        var soapRequest = Control.TraceExtension.XmlRequest.OuterXml;

                        //Se accede a la objeto "XmlResponse" de la clase TraceExtension y llamamos a su propiedad "OuterXml".
                        soapResponse = Control.TraceExtension.XmlResponse.OuterXml;
        ...
        private Boolean valida_duplicidad_claveAcceso(string p_claveAcceso)
        {
            ...
            try
            {
                ...
                DB.Conectar();
                DB.CrearComando("select top 1 claveAcceso from docRecepcion WITH (NOLOCK)  where claveAcceso = @p_claveAcceso and estado='R'");
